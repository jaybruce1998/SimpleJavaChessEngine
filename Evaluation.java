public class Evaluation {
    static class Eval
    {
        ...
        {
            for (int i = 0; i < 64; i++)
                if (board.get(i) != Piece.None)
                    AddPiece(board.get(i), i);
        }
    ...
    private static final int CheckmateBase = 9000;
    private static final int CheckmateScore = 9999;
    ...
    public static boolean IsCheckmate(int score) {
        return Math.abs(score) > CheckmateBase;
    }

    public static int Checkmate(Color color, int ply) {
        return color.value * (ply-CheckmateScore);
    }
    ...
    private static final int[] PhaseValues = {0, 155, 305, 405, 1050, 0};

    private static final int[] MidgameTables = {
              //PAWN MG
    ...
    private static final int[] EndgameTables = {
              //PAWN EG
                    100, 100, 100, 100, 100, 100, 100, 100,
    ...
    public static int[] MobilityValues =
            {
                    //                 opponent                            friendly
    ...
    public static int GetMobility(Board board, int square) {
        ...
        return switch (piece) {
            case Piece.BlackPawn -> GetMobility(board, piece, Attacks.BlackPawn[square]);
            case Piece.WhitePawn -> GetMobility(board, piece, Attacks.WhitePawn[square]);
            case Piece.BlackKing, Piece.WhiteKing -> GetMobility(board, piece, Attacks.King[square]);
            case Piece.BlackKnight, Piece.WhiteKnight -> GetMobility(board, piece, Attacks.Knight[square]);
            case Piece.BlackRook, Piece.WhiteRook -> GetMobilitySlider(board, piece, Attacks.Rook[square]);
            case Piece.BlackBishop, Piece.WhiteBishop -> GetMobilitySlider(board, piece, Attacks.Bishop[square]);
            case Piece.BlackQueen, Piece.WhiteQueen -> GetMobilitySlider(board, piece, Attacks.Queen[square]);
    ...
    private static int GetMobility(Board board, Piece piece, byte[] targets) {
        ...
        //we return negative values if the piece is black (black minimizes)
        return piece.Color().value * result;
    }

    private static int GetMobilitySlider(Board board, Piece piece, byte[][] targets) {
        int result = 0;
        int row = 13 * PieceIndex(piece);
        for (var axis : targets)
            for (var square : axis) {
                Piece targetPiece = board.get(square);
                int offset = targetPiece.IsColor(piece) ? 6 : 0;
                result += MobilityValues[row + offset + Pieces.Order(targetPiece)];
